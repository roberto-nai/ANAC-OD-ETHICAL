...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Cleans a DataFrame by removing columns with the suffix '_y' and renaming columns
    with the suffix '_x' by removing the '_x' suffix.

    Parameters:
    df (pd.DataFrame): The input DataFrame to be cleaned.

    Returns:
    pd.DataFrame: The cleaned DataFrame with '_y' columns removed and '_x' columns renamed.
    """
    # Remove columns with suffix '_y'
    df = df[[col for col in df.columns if not col.endswith('_y')]]
    
    # Rename columns with suffix '_x' by removing the suffix
    df = df.rename(columns=lambda col: col[:-2] if col.endswith('_x') else col)
